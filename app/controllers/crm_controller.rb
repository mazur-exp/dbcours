class CrmController < ApplicationController
  ...
  def update_status
    ...
    # Синхронизируем paid с crm_status В ОБЕ СТОРОНЫ
    ...
    if new_status == 'paid_status'
      ...
      # Если перемещаем ИЗ оплаченных - убираем paid
    ...
    if @user.update(attributes)
      ...
      # Broadcast обновление через ActionCable
      ...
      respond_to do |format|
        format.turbo_stream do
          # Возвращаем обновленную карточку через Turbo Stream
          render turbo_stream: turbo_stream.replace(
            "user_card_#{@user.id}",
            partial: "crm/user_card",
            locals: { user: @user }
          )
        end
        format.json do
