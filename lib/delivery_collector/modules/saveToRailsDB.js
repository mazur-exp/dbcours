...
const { Client, ClientStat } = require('../database/db_rails');

/**
 * Saves restaurant data directly to Rails client_stats table
...
async function saveRestaurantStats(restaurantName, grabStats, gojekStats) {
  try {
    // Find client_id by restaurant name in Rails database
    const client = await Client.findOne({
      where: { name: restaurantName }
    });

    if (!client) {
      console.log(chalk.red(`[Save] ✗ Client "${restaurantName}" not found in Rails DB (skip)`));
      return false;
    }

    console.log(chalk.blue(`[Save] Processing ${restaurantName} (ID: ${client.id})...`));
    ...
    if (grabStats && Array.isArray(grabStats)) {
      grabStats.forEach(stat => {
        ...
        if (!statsByDate[date]) {
          statsByDate[date] = initializeStatRecord(client.id, date);
        }
    ...
    for (const [date, data] of Object.entries(statsByDate)) {
      try {
        ...
        // Upsert to Rails database (equivalent to Rails ClientStat.upsert)
        await ClientStat.upsert(data, {
          conflictFields: ['client_id', 'stat_date']  // Unique constraint
        });
 ...
 */
function initializeStatRecord(clientId, date) {
  return {
    client_id: clientId,
    stat_date: date,
    grab_sales: 0,
