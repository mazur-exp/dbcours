...
async function getMarkedReady(restaurant, from, to) {
  ...
  const createRequestBody = (from, to) => {
    return [
        ...
        `{"size":0,"query":{"bool":{"filter":[{"query_string":{"analyze_wildcard":true,"query":"time:>=${from} AND time:<=${to} AND data.merchant_id: AND NOT id:FP* AND _exists_:data.food_prepared_timestamp"}}]}},"aggs":{"2":{"date_histogram":{"field":"data.restaurant_accepted_timestamp"},"aggs":{}}}}`
    ].join("\n");
  };
  ...
  while (currentDay >= from) {
    ...
    try {
      ...
      if (result) {
        const markedReadyOrders = response.data.responses[0].hits.total.value;
        ...
        if (existingRecord) {
          ...
          existingRecord.marked_ready = markedReadyOrders;
          ...
          await GojekStats.create({
            stat_date: statDate,
            marked_ready: markedReadyOrders,
            restaurant_id: restaurant.id,
          });
        ...
        console.log(chalk.yellow(`отмеченные готовые заказы (Marked Ready): ${markedReadyOrders}`));
