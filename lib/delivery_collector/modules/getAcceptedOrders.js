...
async function getAcceptedOrders(restaurant, from, to) {
  ...
  const createRequestBody = (from, to) => {
    return [
        ...
        `{"size":0,"query":{"bool":{"filter":[{"query_string":{"analyze_wildcard":true,"query":"time:>=${from} AND time:<=${to} AND data.merchant_id: AND NOT id:FP* AND _exists_:data.restaurant_accepted_timestamp"}}]}},"aggs":{"2":{"date_histogram":{"field":"data.restaurant_accepted_timestamp"},"aggs":{}}}}`
    ].join("\n");
  };
  ...
  while (currentDay >= from) {
    ...
    try {
      ...
      if (result) {
        const acceptedOrders = response.data.responses[0].hits.total.value;
        ...
        if (existingRecord) {
          ...
          existingRecord.accepted_orders = acceptedOrders;
        ...
        console.log(chalk.yellow(`Принятые заказы (Accepted Orders): ${acceptedOrders}`));
