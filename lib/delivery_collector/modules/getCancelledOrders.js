...
async function getCancelledOrders(restaurant, from, to) {
  ...
  const createRequestBody = (from, to) => {
    return [
        ...
        `{"size":0,"query":{"bool":{"filter":[{"query_string":{"analyze_wildcard":true,"query":"time:>=${from} AND time:<=${to} AND data.merchant_id: AND NOT id:FP* AND _exists_:data.cancel_reason_code AND data.cancel_reason_code:(\"MERCHANT_ACCEPTANCE_TIMEOUT\" OR \"HIGH_DEMAND\" OR \"MERCHANT_HIGH_DEMAND\" OR \"CCU_PORTAL_MERCHANT_UNCONTACTABLE\" OR \"DCU_PORTAL_MERCHANT_UNCONTACTABLE\" OR \"PORTAL_MERCHANT_UNCONTACTABLE\" OR \"MERCHANT_OTHERS\" OR \"CCU_PORTAL_MERCHANT_OUT_OF_STOCK\" OR \"CUSTOMER_OUT_OF_STOCK\" OR \"DCU_PORTAL_MERCHANT_OUT_OF_STOCK\" OR \"DRIVER_OUT_OF_STOCK\" OR \"ITEMS_OUT_OF_STOCK\" OR \"MCU_PORTAL_MERCHANT_OUT_OF_STOCK\" OR \"MERCHANT_ITEMS_OUT_OF_STOCK\" OR \"PORTAL_MERCHANT_OUT_OF_STOCK\" OR \"CCU_PORTAL_MERCHANT_CLOSED\" OR \"CUSTOMER_STORE_IS_CLOSED\" OR \"DCU_PORTAL_MERCHANT_CLOSED\" OR \"DRIVER_RESTAURANT_MART_CLOSED\" OR \"MCU_PORTAL_MERCHANT_CLOSED\" OR \"MERCHANT_RESTAURANT_CLOSED\" OR \"PORTAL_MERCHANT_CLOSED\" OR \"RESTAURANT_CLOSED\" OR \"CCU_PORTAL_MERCHANT_WRONG_PRICE\" OR \"DCU_PORTAL_MERCHANT_WRONG_PRICE\" OR \"MCU_PORTAL_MERCHANT_WRONG_PRICE\" OR \"PORTAL_MERCHANT_WRONG_PRICE\")"}}]}},"aggs":{"2":{"date_histogram":{"field":"time"},"aggs":{}}}}`
    ].join("\n");
  };
  ...
  while (currentDay >= from) {
    ...
    try {
      ...
      if (result) {
        const cancelledOrders = response.data.responses[0].hits.total.value;
        ...
        if (existingRecord) {
          ...
          existingRecord.cancelled_orders = cancelledOrders;
        ...
        console.log(chalk.yellow(`Отмененные заказы (Cancelled Orders): ${cancelledOrders}`));
